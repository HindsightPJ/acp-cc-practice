...
class PracticeMode(BaseMode):
    ...
    def _setup_mode_ui(self):
        ...
        content_frame = tk.Frame(self, bg=theme.BG_PAGE)
    ...
    def _create_options_card(self, content_frame):
        ...
        options_card = tk.Frame(
            content_frame, bg=theme.BG_CARD, highlightbackground=theme.BORDER, highlightthickness=1
        )
    ...
    def _create_action_bar(self, content_frame):
        """操作栏：提交/上一题/下一题 + 统计标签。"""
        action_bar = tk.Frame(content_frame, bg=theme.BG_PAGE)
    ...
    def _create_result_card(self, content_frame):
        ...
        res_inner = tk.Frame(result_card, bg=theme.BG_CARD)
        res_inner.pack(fill=tk.BOTH, expand=True, padx=20, pady=14)

        res_top = tk.Frame(res_inner, bg=theme.BG_CARD)
