...
class Header(tk.Frame):
    ...
    def __init__(self, parent, license_status=None, total_count: int = 0,
                 trial_count: int = 0,
                 on_activate_click: Optional[Callable[[], None]] = None,
                 mode_title: str = '练习') -> None:
        """初始化 Header。
        ...
        Args:
            ...
            mode_title: 初始模式标题
        """
        super().__init__(parent, bg=BG_CARD, height=56)
    ...
    def update_stats(self, practiced: int, accuracy: float, wrong_count: int) -> None:
        ...
        self.stats_label.configure(
            text=f"已练 {practiced} 题  ·  正确率 {accuracy:.0f}%  ·  错题 {wrong_count}")

    def set_license_status(self, license_status, total_count: Optional[int] = None,
                           trial_count: Optional[int] = None) -> None:
        """更新授权状态显示。
